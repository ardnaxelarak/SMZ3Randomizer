        ...
        public Pink(World world, Config config) : base(world, config) {
            ...
            Locations = new List<Location> {
                new Location(this, 14, 0x8F84E4, LocationType.Chozo, "Super Missile (pink Brinstar)", Logic switch {
                    Hard => new Requirement(items => (items.CardBrinstarBoss || items.CardBrinstarL2) && items.CanPassBombPassages() && items.Super),
                    _ => new Requirement(items => items.CardBrinstarBoss && items.CanPassBombPassages() && items.Super)
                }),
                ...
                new Location(this, 24, 0x8F865C, LocationType.Visible, "Power Bomb (pink Brinstar)", Logic switch {
                    Hard => new Requirement(items => items.CanUsePowerBombs() && items.Super),
                    _ => items => items.CanUsePowerBombs() && items.Super && items.HasEnergyReserves(1)
                }),
                ...
                new Location(this, 35, 0x8F8824, LocationType.Visible, "Energy Tank, Brinstar Gate", Logic switch {
                    Hard => new Requirement(items => items.CardBrinstarL2 && items.CanUsePowerBombs() && (items.Wave || items.Super)),
                    _ => items => items.CardBrinstarL2 && items.CanUsePowerBombs() && items.Wave && items.HasEnergyReserves(1)
                }),
            };
        }
            ...
            return Logic switch {
                Hard =>
                    ...
                    items.CanAccessNorfairUpperPortal() && items.Morph && (items.Missile || items.Super || items.Wave /* Blue Gate */) &&
                        (items.Ice || items.HiJump || items.CanSpringBallJump() || items.CanFly(Config)),
                _ =>
                    items.CanOpenRedDoors() && (items.CanDestroyBombWalls() || items.SpeedBooster) ||
                    items.CanUsePowerBombs() ||

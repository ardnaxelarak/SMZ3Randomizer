        ...
        public East(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                ...
                new Location(this, 75, 0x8F8FD2, LocationType.Visible, "Power Bomb (lower Norfair above fire flea room)", Logic switch {
                    Hard => items => CanExit(items) && items.CanPassBombPassages(),
                    _ => new Requirement(items => CanExit(items)),
                }),
                ...
                new Location(this, 77, 0x8F9100, LocationType.Visible, "Missile (lower Norfair near Wave Beam)", Logic switch {
                    Hard => items => CanExit(items) && items.Morph && items.CanDestroyBombWalls(),
                    _ => new Requirement(items => CanExit(items)),
                }),
            ...
            return Logic switch {
                Hard =>
                    items.Varia && items.CardLowerNorfairL1 && (
                        ...
                        items.CanAccessNorfairLowerPortal() && items.CanDestroyBombWalls() && items.Super && (items.CanFly(Config) || items.CanSpringBallJump() || items.SpeedBooster)
                    ) &&
                ...
                _ =>
                    items.Varia && items.CardLowerNorfairL1 && (
                        ...
                        items.CanAccessNorfairLowerPortal() && items.CanDestroyBombWalls() && items.Super && items.CanUsePowerBombs() && items.CanFly(Config)
                    ),
            };
        }

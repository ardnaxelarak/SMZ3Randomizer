        ...
        public Central(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                new Location(this, 0, 0x8F81CC, LocationType.Visible, "Power Bomb (Crateria surface)", Logic switch {
                    _ => new Requirement(items => (config.Keysanity ? items.CardCrateriaL1 : items.CanUsePowerBombs()) && (items.SpeedBooster || items.CanFly(Config)))
                }),
                ...
                new Location(this, 7, 0x8F8404, LocationType.Chozo, "Bombs", Logic switch {
                    Hard => new Requirement(items => (config.Keysanity ? items.CardCrateriaBoss : items.CanOpenRedDoors()) && items.Morph),
                    _ => items => (config.Keysanity ? items.CardCrateriaBoss : items.CanOpenRedDoors()) && items.CanPassBombPassages(),
                })
            };
        }

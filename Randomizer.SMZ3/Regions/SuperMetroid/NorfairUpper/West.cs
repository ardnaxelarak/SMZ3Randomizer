        ...
        public West(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                new Location(this, 49, 0x8F8AE4, LocationType.Hidden, "Missile (lava room)", Logic switch {
                    Hard => new Requirement(items => items.CanHellRun() && (
                    ...
                    _ => items => items.Varia && (
                            items.CanOpenRedDoors() && (items.CanFly(Config) || items.HiJump || items.SpeedBooster) ||
                            World.CanEnter("Norfair Upper East", items) && items.CardNorfairL2
                        ) && items.Morph,
                }),
                new Location(this, 50, 0x8F8B24, LocationType.Chozo, "Ice Beam", Logic switch {
                    Hard => new Requirement(items => (config.Keysanity ? items.CardNorfairL1 : items.Super) && items.Morph && (items.Varia || items.HasEnergyReserves(3))),
                    _ => items => (config.Keysanity ? items.CardNorfairL1 : items.Super) && items.CanPassBombPassages() && items.Varia && items.SpeedBooster,
                }),
                new Location(this, 51, 0x8F8B46, LocationType.Hidden, "Missile (below Ice Beam)", Logic switch {
                    ...
                    _ => items => (config.Keysanity ? items.CardNorfairL1 : items.Super) && items.CanUsePowerBombs() && items.Varia && items.SpeedBooster,
                }),
